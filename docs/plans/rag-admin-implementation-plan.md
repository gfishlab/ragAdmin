...
### 8.5.1 OCR 与文档解析技术选型说明

先统一术语：

- `Apache Tika` 是通用文档文本提取器，不是 OCR 引擎
- `Tesseract` 是 OCR 引擎，负责从图片像素或扫描件中识别文字
- `Tess4J` 是 Java 对 `Tesseract` 的封装，不是独立 OCR 引擎
- `MinerU` 是面向复杂文档的高阶解析方案，不只是 OCR；它既能处理文本型 `PDF`，也能在需要时走 OCR / 视觉识别链路

在 RAG 文档入库链路中，需要先区分“文本提取”和“OCR”：

- 文本型 `PDF` 通常不需要 OCR，直接抽取文本层即可
- 扫描版 `PDF`、`PNG`、`JPG`、`JPEG`、`WEBP` 等如果要提取文字，则必须具备 OCR 或等价视觉识别能力
- 因此“系统支持解析 `PDF` / 图片”不等于“所有场景都必须安装单独 OCR 引擎”，但“扫描件和图片要抽字”一定要依赖某种 OCR 类能力

当前项目阶段方案：

1. 一期采用 `Tika + Tesseract`
2. 二期在需要更高质量复杂文档解析时，引入 `MinerU` 独立解析服务

采用该方案的原因：

- `Tika + Tesseract` 足以覆盖首期大多数文本型文档、普通图片和扫描版 `PDF`
- 当前后端是 `Java + Spring Boot`，而 `MinerU` 更适合作为独立解析服务或异步 worker 接入，不适合直接塞进主进程
- 先把上传、解析、切片、Embedding、索引构建主链路稳定跑通，比一开始就引入重型解析栈更符合首期目标

当前项目对 OCR 的准确表述应为：

- 项目已经内建 OCR 调用链路和解析编排能力
- 但一期 OCR 引擎本体仍属于外部运行依赖，需要运维环境单独提供 `Tesseract`
- 本地 Windows 开发可以配置 `tesseract.exe`
- 生产 Linux 部署必须切换为 Linux 命令或 Linux 路径

二期 `MinerU` 的接入定位：

- 作为独立文档解析服务或解析 worker，对外暴露内部 `HTTP/RPC` 或任务投递接口
- 用于复杂版式 `PDF`、多栏文档、表格、公式、版面顺序恢复等高要求场景
- 不把“人工先跑 `MinerU` 再上传产物”作为标准使用流程，仍坚持“上传原始文件，由系统统一异步解析”

常见候选方案可分为三类：

- 轻量 OCR 引擎：`Tesseract`
- OCR / 文档解析综合方案：`PaddleOCR`、`MinerU`
- Java 调用封装：`Tess4J`
