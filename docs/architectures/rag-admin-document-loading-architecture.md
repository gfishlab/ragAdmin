...
### 5.5 DOCX / PPTX — MinerU 原生解析

- **策略**：`OfficeToMineruReaderStrategy`（`@Order(40)`）
- **引擎**：MinerU API（原生支持 DOCX/PPTX）
- **输出**：Markdown 格式的 `Document` 列表
- **parseMode**：`OCR`
- **readerType**：`OFFICE_MINERU`
...
```
DOCX/PPTX 文件 → 上传 MinIO → 生成预签名 URL → MinerU API 解析 → Markdown
```

**关键设计决策：**

- MinerU 原生支持 DOCX/PPTX，无需客户端预先转换为 PDF
- 上传原始 Office 文件到 MinIO，由 MinerU 服务端自行处理
- MinerU 不可用或解析失败时降级到 Tika
...
### 7.2 OfficeToMineruReaderStrategy — Office 文档直接 MinerU

**职责**：将 DOCX/PPTX 原始文件直接提交给 MinerU API 解析
...
2. read():
   a. 检查 MinerU 是否可用，不可用则降级到 Tika
   b. 将原始 Office 文件上传到 MinIO
   c. 生成预签名 URL，调用 MineruParseService.parseByUrlWithImages() 解析
   d. 补充 metadata: readerType=OFFICE_MINERU, parseMode=OCR, originalDocType=DOCX/PPTX
3. 降级: MinerU 不可用或解析失败时，回退到 Tika
```

**MinerU 原生支持 Office 格式**：无需客户端预先转换为 PDF，MinerU 服务端会自行处理格式转换。
...
| DOCX/PPTX | Tika `parseToString()`（纯文本） | MinerU 原生解析（Markdown） |
