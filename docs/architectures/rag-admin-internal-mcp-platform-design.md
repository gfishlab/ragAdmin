...
- 企业已大量使用本地 OCR 二进制程序，且短期无法迁移到官方 API 或内部服务
- 短期内无法把 OCR 能力服务化
...
- OCR 仍然归属于文档解析链路
- 不引入 `stdio`
- 默认由系统在文档上传后自动调用 `MinerU API` 完成 OCR / 结构化解析与后续入库，不把人工先跑 `MinerU` 等工具作为标准前置流程
- 当前标准主链路不再依赖本地 `Tesseract` CLI
- 如果后续需要私有化或更强控制能力，可将 `MinerU` 部署为内部服务，但仍保持在 `Knowledge Pipeline` 内部
