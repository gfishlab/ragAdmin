...
- 已完成主线：认证授权、用户管理、模型管理、文档处理管线（含图片处理 + 内容感知清洗）、混合检索、RAG SSE 聊天、会话记忆三层架构（PG长期+Redis短期+摘要压缩）、后台治理、管理前端、聊天前端
- 进行中重点：聊天记忆收尾（摘要触发阈值生效+刷新日志）、查询改写接入检索管线、语义/父子分块、Cross-Encoder 重排序、Dashboard 扩展、分块策略 YAML 参数覆写
