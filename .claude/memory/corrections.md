...
### [A-009] 跳过 plans 文档直接编码

- 日期：2026-04-26
- 模式描述：架构设计讨论完成后，跳过 `docs/plans/` 下的实施计划文档，直接进入编码。plan 文档还写到了 `~/.claude/plans/` 而不是项目 `docs/plans/`
- 风险：用户无法审阅实施步骤，实施偏离设计意图，文档流转链路断裂
- 更优做法：严格遵守 `docs/architectures → docs/plans → 编码` 的流转顺序。用户说"开始执行"也是指执行 plans 里规划好的步骤
- 适用范围：所有涉及多文件、多步骤的非简单实施任务
